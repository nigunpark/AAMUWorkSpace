...
<div class="main-panel">
	...
	<form id="form" class="form-inline col-sm-12 d-flex justify-content-center mt-3">
		...
		<input type="text" id="answer" name="answer" class="form-control mx-2 w-50"
				...
				<tbody class="table-sm down-file-body" id="answer-list">
						...
						<c:if test="${not isEmpty }">	
							<c:forEach items="${record.answer }" var="item">
								...
								<c:set var="titleClass" value="${sessionScope.id==item.id ? 'answer' : ''}"/>						
								<c:set var="inactive" value="${sessionScope.id==item.id ? '' : 'disabled'}"/>
								<tr>
									<td>${item.name }</td>
									<td  class="text-left ${titleClass}" title="${item.qno}">${item.answer }</td>
...
$('#submit').click(function(){
	...
	$.ajax({
		...
		console.log("입력한 댓글:",{id:"${sessionScope.id}",lineComment:$("#answer").val(),lpostDate:new Date().getTime(),name:"코스모",no:${record.qno}});
		...
		if($('#submit').val()==="등록"){
			var tr="<tr><td>"+data.name+"</td><td title=\""+data.qno+"\" class=\"text-left linecomment\">"+$("#answer").val()+"</td><td>"+(getDate(new Date().getTime()))+"</td><td><span class=\"btn btn-info btn-sm delete\">삭제</span></td></tr>";
			$('#answer-list').before(tr);
		}
		else{//수정
			$('#submit').val("등록");
			$('td[title='+data.qno+']').html(data.answer);
			console.log("수정하려는 댓글 번호:",data.qno);
...
$(document).on('click','.answer',function(){
	...
	console.log("qno히든값 설정 확인:",$('input[name=qno]').val());
});
...
function showComments(){
	$.ajax({
		url:"<c:url value="/qna/answer/List.do"/>",
		data:{"qno":"${record.qno}"},
		dataType:"json",
		type:"post",
